...
GraphicsManager3D::~GraphicsManager3D() {
  LOG_DEBUG_SYS(RENDERING, "Destroying synchronization objects...");
...
void GraphicsManager3D::gameLoopUpdate(const time_ms duration) {
  (void)duration;
  this->drawFrame();
}
...
void GraphicsManager3D::createSyncObjects() {
  ...
  if (vkCreateSemaphore(this->_device, &semaphoreInfo, nullptr, &this->_imageAvailableSemaphore) != VK_SUCCESS ||
      vkCreateSemaphore(this->_device, &semaphoreInfo, nullptr, &this->_renderFinishedSemaphore) != VK_SUCCESS ||
      vkCreateFence(this->_device, &fenceInfo, nullptr, &this->_inFlightFence) != VK_SUCCESS) {
    throw std::runtime_error("failed to create semaphores!");
  }
}

void GraphicsManager3D::drawFrame() {
  vkWaitForFences(this->_device, 1, &this->_inFlightFence, VK_TRUE, UINT64_MAX);
  vkResetFences(this->_device, 1, &this->_inFlightFence);

  uint32_t imageIndex;
  vkAcquireNextImageKHR(this->_device, this->_swapChain, UINT64_MAX, this->_imageAvailableSemaphore, VK_NULL_HANDLE,
                        &imageIndex);

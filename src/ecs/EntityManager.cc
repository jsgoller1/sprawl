...
EntityManager::EntityManager() : entityCount(0) {
  this->identityToEntity = shared_ptr<map<shared_ptr<Identity>, Entity*>>(new map<shared_ptr<Identity>, Entity*>());
  this->entityToIdentity = shared_ptr<map<Entity*, shared_ptr<Identity>>>(new map<Entity*, shared_ptr<Identity>>());
};
...
// Private
shared_ptr<Identity> EntityManager::manage(Entity* entity, const EntityName& name) {
  ...
  this->identityToEntity->insert(pair<shared_ptr<Identity>, Entity*>(identity, entity));
  this->entityToIdentity->insert(pair<Entity*, shared_ptr<Identity>>(entity, identity));
...
shared_ptr<Identity> EntityManager::createIdentity(const EntityName name) {
  this->entityCount++;
  EntityName actualName = fmt::format("{0}-{1}", (name.size() ? name : "Entity-"), to_string(this->entityCount));

  shared_ptr<EntityID> entityID = shared_ptr<EntityID>(new EntityID(actualName));
  return shared_ptr<Identity>(new Identity(entityID));
}

shared_ptr<set<Entity*>> EntityManager::getAllEntities() {
  shared_ptr<set<Entity*>> entities = shared_ptr<set<Entity*>>(new set<Entity*>());

...
CC_COMPILER:=clang++
CC_FLAGS:=-std=c++20 -g -O4
WARNINGS:=-Weverything -Werror
INCLUDE:=-I src/include -I src/3rdparty/include -I src/components/include
INCLUDE:=$(INCLUDE) -I src/ecs/include -I src/math/include -I src/physics/include  -I src/wads/include 
LINK:=-lSDL2 -lSDL2_image
CLANG_LINKER_ARGS:=-Wl,-rpath -Wl,/usr/local/lib/ # For libsdl2_image
SOURCE:=src/*.cc src/components/*.cc src/ecs/*.cc src/math/*.cc src/physics/*.cc src/wads/*.cc
